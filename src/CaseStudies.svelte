...
function handleExport(columns) {
    const csvData = hypotheses.map((hypothesis) => {
        ...
        if (columns.detail.includes("Hypothesis")) {
            data.Hypothesis = hypothesis.text;
        }

        if (columns.detail.includes("Consequences")) {
        ...
        if (columns.detail.includes("Action")) {
            data.Action = hypothesis.action;
        }

        if (columns.detail.includes("Measure")) {
            data.Measure = hypothesis.measure;
        }

        if (columns.detail.includes("Timescale")) {
    ...
    if (columns.detail.includes("Consequences")) {
        consequences.forEach((consequence) => {
            let data = {
                ...
                Consequences: `${consequence.description}, ${consequence.type}, ${consequence.priority}, ${consequence.outcome}, ${consequence.AIM}`
            };
            csvData.push(data);
        });
    }
      ...
      <div class="consequence-options">
        ...
        <div class="input-row">
          <label for="timescale">Timescale
            <select bind:value={hypothesis.timescale} on:select={(event) => setTimelineForHypothesis(i, event.target.value)}>
...
.consequence-options .consequence-input {
    font-size: 15px;
    margin-bottom: 10px; /* Changed from margin-top */
    padding: 3px;
}
...
/* get timescale label and set to display: block */
  .timescale-label {
    display: block;
  }
